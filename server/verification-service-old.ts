...
export class VerificationService {
  ...
  async verifyContact(
    ...
    return { success: false, message: "Método de verificación no configurado" };
  }

  // Check if user needs verification for specific action
  async requiresVerification(userId: string, action: string): Promise<boolean> {
    const [user] = await db
      .select()
      .from(users)
    ...
    const sensitiveActions = [
      "admin_access",
      "large_order", // Orders over certain amount
      "profile_change",
  ...
  async updateTwoFactorSettings(
    ...
    try {
      ...
      const statusMessage = enabled 
        ? `Verificación de seguridad activada${method ? ` por ${method}` : ""}`
  ...
  // Verify phone or email for initial setup
  async verifyContact(
    userId: string, 
    type: "phone" | "email", 
    code: string
    ...
    if (verification.success) {
      const updateField = type === "phone" ? "isPhoneVerified" : "isEmailVerified";
